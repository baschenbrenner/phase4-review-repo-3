...
function ClientCard(props) {
  ...
  const invoicesToDisplay = props.invoices.map((inv) => {
    if (inv.user_id === user.id) {
      return (
        <Grid item xs={12} id={inv.id} key={inv.id} className="invoice-tag">
          ...
          <Typography variant="subtitle1" component="h6">
            Date Invoice Sent: {inv.date_invoice_sent}
          </Typography>
          <Typography variant="subtitle1" component="h6">
            Date Invoice Paid:{" "}
            {inv.date_invoice_paid === null
              ? "Not yet paid"
          ...
          <Button
            id={inv.id}
            variant="outlined"
            onClick={(e) => props.handleDeleteInvoice(e.target.id)}
          >
            X
          </Button>
        </Grid>
  ...
  return (
    <Paper elevation={4} className="client-card">
      ...
      <Typography variant="subtitle1" component="h6">
        Point of Contact: {props.point_of_contact}
      </Typography>
      <Typography variant="subtitle2" component="h6">
        Email: {props.poc_email}
      </Typography>
      ...
      <Grid container>{showInvoices ? invoicesToDisplay : null}</Grid>
    </Paper>
  );
}
